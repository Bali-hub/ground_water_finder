...
# Forcer le backend non-interactif pour Docker
matplotlib.use("Agg")
# ==========================================================================
...
# ================== CHEMINS PORTABLES ==================
PROJECT_ROOT = Path(__file__).resolve().parent.parent  # remonte depuis utils/
BASE_CLIENTS = PROJECT_ROOT / "data" / "Dossier_clients"


# ================== FONCTIONS ==================
def choisir_dossier_client():
    """Sélection automatique du dernier client"""
    if not BASE_CLIENTS.exists():
        st.error(f"❌ Le dossier {BASE_CLIENTS} n'existe pas.")
        st.stop()

    dossiers = [d for d in BASE_CLIENTS.iterdir() if d.is_dir()]
    if not dossiers:
        st.error(f"❌ Aucun dossier client trouvé dans {BASE_CLIENTS}")
        st.stop()

    # Choix automatique du dernier dossier
    nom_client = sorted(dossiers, reverse=True)[0].name
    dossier_client = BASE_CLIENTS / nom_client
    st.success(f"📁 Client détecté automatiquement : {nom_client}")
    return dossier_client, nom_client


def load_contour(file_path: Path):
    """Charge un polygone à partir de GPX, KML ou KMZ"""
    ...
    if ext == ".gpx":
        ...
        for placemark in root.xpath(".//kml:Placemark", namespaces=ns):
            for poly_elem in placemark.xpath(".//kml:Polygon", namespaces=ns):
                coords_text = poly_elem.xpath(
                    ".//kml:coordinates/text()", namespaces=ns
                )
                for c in coords_text:
                    coords = [
                        (float(lon), float(lat))
                        for lon, lat, *_ in (p.split(",") for p in c.strip().split())
                    ]
                    if len(coords) >= 3:
...
def charger_couche(path: Path):
    """Charge un fichier SHP, GPX, KML ou KMZ en GeoDataFrame"""
...
def generer_journal(couches, output_path: Path):
    with output_path.open("w", encoding="utf-8") as f:
        ...
        for nom, gdf in couches.items():
            f.write(
                f"{nom.upper():<15}: {len(gdf) if gdf is not None else 0} entités\n"
            )
        f.write("=" * 60 + "\n")
        f.write("Remarques:\n")
...
def shp_to_kml_gpx_kmz(gdf, basename, target_dirs):
    """Convertit un GeoDataFrame en KML, KMZ et GPX"""
    ...
    kml_path = target_dirs["kml"] / f"{basename}.kml"
    kml.save(kml_path)

    kmz_path = target_dirs["kmz"] / f"{basename}.kmz"
    ...
    gpx_path = target_dirs["gpx"] / f"{basename}.gpx"
...
def main():
    ...
    contour_input_path = next(
        f
        for f in input_dir.iterdir()
        if "contour" in f.name.lower() and f.suffix.lower() in [".gpx", ".kml", ".kmz"]
    )
    contour_poly = load_contour(contour_input_path)
    contour_gdf = gpd.GeoDataFrame(geometry=[contour_poly], crs="EPSG:4326").to_crs(
        epsg=3857
    )

    # RENDU – Autres couches
    ...
    if couches.get("lines") is not None:
        couches["lines"].plot(
            ax=ax, color="orange", linewidth=3.0, linestyle="--", alpha=0.8, zorder=6
        )
    handles.append(
        mpatches.Patch(
            facecolor="orange", edgecolor="orange", label="Fractures identifiées"
        )
    )
    ...
    handles.append(
        mpatches.Patch(facecolor="lime", edgecolor="black", label="Points de forage")
    )

    couches["contour"].plot(
        ax=ax, facecolor="none", edgecolor="darkblue", linewidth=3, alpha=0.9, zorder=5
    )
    handles.append(
        mpatches.Patch(
            facecolor="none",
    ...
    for i, source in enumerate(satellite_sources):
        try:
            ...
            st.success(
                f"✅ Fond de carte satellite chargé (source {i+1}/{len(satellite_sources)})"
            )
            satellite_loaded = True
            break
        except Exception as e:
            if i < len(satellite_sources) - 1:
                st.warning(
                    f"⚠️ Source satellite {i+1} indisponible, tentative suivante..."
                )
            continue

    # Si aucun fond satellite n'a fonctionné, essayer OpenStreetMap
    if not satellite_loaded:
        try:
            ...
            st.success("✅ Fond de carte OpenStreetMap chargé (fallback)")
        except Exception as e2:
            st.error(f"❌ Aucun fond de carte disponible")
            # Fond de secours simple
            ax.set_facecolor("#e8f4f8")  # Bleu clair pour l'eau
            # Ajouter un quadrillage léger
            ax.grid(True, alpha=0.3, linestyle="--", linewidth=0.5)
            ax.text(
                0.5,
                0.5,
                "Fond de carte non disponible\nVérifiez la connexion internet",
    ...
    ax.set_axis_off()
    ax.set_title(
        f"🗺️ Carte de prospection – Projet {nom_client}", fontsize=18, weight="bold"
    )
    ax.legend(
        handles=handles, loc="lower left", fontsize=10, frameon=True, framealpha=0.95
    )
    plt.tight_layout()

    # ================== EXPORT ==================
    rapport_dir = dossier_RENDU / f"Rapport_{nom_client}"
    for d in [rapport_dir]:
        d.mkdir(parents=True, exist_ok=True)

    # MODIFICATION CRITIQUE POUR DOCKER: Sauvegarder AVANT d'afficher avec Streamlit
    fig_path = rapport_dir / "carte_prospection.png"

    # Améliorer les paramètres de sauvegarde pour Docker
    fig.savefig(
        fig_path,
        dpi=150,
        bbox_inches="tight",  # Éviter les marges blanches
        pad_inches=0.1,  # Padding minimal
        facecolor="white",  # Fond blanc garanti
        edgecolor="none",  # Pas de bordure
    )

    # Afficher la carte avec des paramètres optimisés pour Docker
    st.pyplot(fig, clear_figure=True)  # clear_figure=True libère la mémoire
    ...
    shutil.make_archive(
        base_name=str(zip_path.with_suffix("")), format="zip", root_dir=rapport_dir
    )
...
def main():
    ...
    try:
        ctx.add_basemap(
            ax,
            source="http://mt1.google.com/vt/lyrs=y&x={x}&y={y}&z={z}",
            crs="EPSG:3857",
            zorder=0,
        )
    except:
        ctx.add_basemap(
            ax, source=ctx.providers.OpenStreetMap.Mapnik, crs="EPSG:3857", zorder=0
        )
